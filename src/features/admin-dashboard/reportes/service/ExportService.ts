...
export const ExportService = {
  ...
  exportVacunasByDueno: async (duenoId: string): Promise<ExportDataResponse> => {
    try {
      const response = await apiClient.get(`${ENDPOINTS.admin.reportes.exportByDuenoId(duenoId)}`, {
        responseType: 'blob',
        headers: {
  ...
  exportAllVacunas: async (anio: number): Promise<ExportDataResponse> => {
    try {
      const response = await apiClient.get(`${ENDPOINTS.admin.reportes.exportAllData}?anio=${anio}`, {
        responseType: 'blob',
        headers: {
          'Accept': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
        }
