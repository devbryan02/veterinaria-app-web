...
export default function SelectorMap({ 
  ...
  if (!isClient) {
    return (
      <div 
        className="w-full rounded-lg flex items-center justify-center bg-muted"
        style={{ height }}
      >
        <div className="text-muted-foreground">Cargando mapa...</div>
      </div>
    ...
    <div className="space-y-3 h-full flex flex-col">
      <div className="flex items-center justify-between shrink-0">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 h-8 px-3 text-xs border rounded-md hover:bg-accent disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
        >
      ...
      <div className="relative z-50 shrink-0" ref={searchContainerRef}>
        <div className="relative">
          <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 h-3 w-3 text-muted-foreground z-10" />
          <Input
            type="text"
            ...
            className="pl-9 pr-8 relative z-10 h-8 text-sm"
          />
          {searchQuery && (
            <button
              type="button"
              onClick={clearSearch}
              className="absolute right-1 top-1/2 transform -translate-y-1/2 h-6 w-6 p-0 z-10 hover:bg-accent rounded transition-colors flex items-center justify-center"
            >
        ...
        {/* Resultados de búsqueda - AQUÍ ESTÁ EL FIX */}
        {showResults && (
          <div className="absolute top-full left-0 right-0 z-[999] mt-1 bg-background border border-border rounded-md shadow-xl max-h-40 overflow-y-auto">
            {isSearching && (
              <div className="p-4">
              ...
              <>
                {searchResults.map((result, index) => (
                  <button
                    ...
                    className="w-full text-left px-4 py-3 hover:bg-accent border-b border-border last:border-b-0 transition-colors focus:bg-accent focus:outline-none"
                  >
      ...
      <div className="border border-border rounded-lg overflow-hidden relative z-10 flex-1 min-h-0">
